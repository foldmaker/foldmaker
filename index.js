...
export let tokenize = (string, TOKENS, CALLBACK) => {
  ...
  while (string) {
    for (let i = 0; i < len; i += 1) {
      ...
      tokens.push(
        CALLBACK ? CALLBACK({ type, value, m, index }) : { type, value }
      );
...
export let direct = (array, directives, self) => ({
  type,
  value,
  m,
  index
  ...
  if (directives[type]) {
    ...
    let returnValue = directives[type](result);
    if (returnValue) self.add(returnValue, "o");
  } else {
    self.noop(result);
  }
};
